...
public class TokenServiceImpl implements TokenService {
	...
	public String validateToken(String header) throws Exception {
		...
		String tokenHeaderPayloadHash = hmacEncode.HmacAndBase64(HmacEncode.SECRET_KEY, tokenHeader+"."+tokenPayload, "HmacSHA256");
		String tokenSignatureDecode = new String(Base64.getDecoder().decode(tokenSignature));

		if (tokenHeaderPayloadHash.equals(tokenSignatureDecode)) {
			return token;
		}
	...
	public boolean validateTokenExpiration(String token) throws Exception {
		...
		String tokenPayloadDecode = new String(Base64.getDecoder().decode(tokenPayload));
